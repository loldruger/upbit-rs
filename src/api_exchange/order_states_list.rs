...
impl OrderInfo {
    pub async fn get_order_states_by_uuids(market_id: &str, uuids: &[&str], order_by: OrderBy) -> Result<Vec<Self>, ResponseError> {
    ...
    pub async fn get_order_states_by_identifiers(market_id: &str, identifiers: &[&str], order_by: OrderBy) -> Result<Vec<Self>, ResponseError> {
    ...
    pub async fn get_order_states_opened(market_id: &str, state: OrderState, states: &[OrderState], page: u8, limit: u8, order_by: OrderBy) -> Result<Vec<Self>, ResponseError> {
        let res = Self::request_get_orders_opened(market_id, state, states, page, limit, order_by).await?;
        let res_serialized = res.text().await.map_err(crate::response::response_error_from_reqwest)?;
    ...
    async fn request_get_orders_by_uuids(market_id: &str, uuids: &[&str], order_by: OrderBy) -> Result<Response, ResponseError> {
    ...
    async fn request_get_orders_by_identifiers(market_id: &str, identifiers: &[&str], order_by: OrderBy) -> Result<Response, ResponseError> {
        let mut url = Url::parse(&format!("{URL_SERVER}{URL_ORDER_STATUS_BY_UUID}")).unwrap();
    ...
    async fn request_get_orders_opened(market_id: &str, state: OrderState, states: &[OrderState], page: u8, limit: u8, order_by: OrderBy) -> Result<Response, ResponseError> {
...
mod tests {
    ...
    async fn test_get_orders_by_uuids() {
        ...
        let res = OrderInfo::request_get_orders_by_uuids("KRW-ETH", &[uuid.as_str()], OrderBy::Desc).await.unwrap();
        let res_serialized = res.text().await.map_err(crate::response::response_error_from_reqwest).unwrap();
        ...
        if let Some(json_array) = json.as_array() {
            for (index, item) in json_array.iter().enumerate() {
                ...
                if !missing_keys.is_empty() {
                    println!("[test_get_orders_by_uuids] Missing keys in item[{}]: {:?}", index, missing_keys);
                    assert!(false);
                } else {
                    println!("[test_get_orders_by_uuids] No keys are missing in item[{}]", index);
                    assert!(true);
                }
    
                if !extra_keys.is_empty() {
                    println!("[test_get_orders_by_uuids] Extra keys in item[{}]: {:?}", index, extra_keys);
                    assert!(false);
                } else {
                    println!("[test_get_orders_by_uuids] No extra keys found in item[{}]", index);
                    assert!(true);
                }
            }
        } else {
    ...
    async fn test_get_order_states_opened() {
        ...
        let res = OrderInfo::request_get_orders_opened("KRW-ETH", OrderState::Wait, &[OrderState::Wait], 1, 10, OrderBy::Desc).await.unwrap();
        let res_serialized = res.text().await.map_err(crate::response::response_error_from_reqwest).unwrap();
    ...
    let expected_structure = expected_structure[0]
        ...
        if let Some(json_array) = json.as_array() {
            for (index, item) in json_array.iter().enumerate() {
                ...
                if !missing_keys.is_empty() {
                    println!("[test_get_order_states_opened] Missing keys in item[{}]: {:?}", index, missing_keys);
                    assert!(false);
                } else {
                    println!("[test_get_order_states_opened] No keys are missing in item[{}]", index);
                    assert!(true);
                }
    
                if !extra_keys.is_empty() {
                    println!("[test_get_order_states_opened] Extra keys in item[{}]: {:?}", index, extra_keys);
                    assert!(false);
                } else {
                    println!("[test_get_order_states_opened] No extra keys found in item[{}]", index);
                    assert!(true);
                }
            }
        } else {
