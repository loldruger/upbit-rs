...
use super::{
    ...
    super::response::{
        AccountsInfo, ObjectAskBid, ObjectMarket, OrderChance, OrderChanceSource, ResponseError,
    },
};
                ...
                Self {
                    ...
                    market: ObjectMarket {
                        ...
                        ask_types: x
                            .market
                            .ask_types
                            .map(|x| x.into_iter().map(|x| x.as_str().into()).collect()),
                        bid_types: x
                            .market
                            .bid_types
                            .map(|x| x.into_iter().map(|x| x.as_str().into()).collect()),
...
mod tests {
    ...
    async fn test_get_order_chance() {
        ...
        crate::test_utils::check_error_response(&res_serialized, "test_get_order_chance");
