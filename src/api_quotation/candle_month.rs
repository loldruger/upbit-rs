...
mod tests {
    ...
    #[tokio::test]
    async fn test_request_candle_month() {
        ...
        if let Some(json_array) = json.as_array() {
            for (index, item) in json_array.iter().enumerate() {
                ...
                if !missing_keys.is_empty() {
                    println!("[test_request_candle_month] Missing keys in item[{}]: {:?}", index, missing_keys);
                    assert!(false);
                } else {
                    println!("[test_request_candle_month] No keys are missing in item[{}]", index);
                    assert!(true);
                }
    
                if !extra_keys.is_empty() {
                    println!("[test_request_candle_month] Extra keys in item[{}]: {:?}", index, extra_keys);
                    assert!(false);
                } else {
                    println!("[test_request_candle_month] No extra keys found in item[{}]", index);
                    assert!(true);
                }
            }
        } else {
