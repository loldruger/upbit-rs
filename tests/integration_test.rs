use upbit::{self, api_exchange::{OrderSide, OrderType}, api_quotation::CandleMinute};
use tokio;
...
async fn test_get_candle_of_minute() {
    ...
    let candle_1 = upbit::api_quotation::get_candle_minute("KRW-ETH", None, 1, CandleMinute::Min1).await;

    assert!(candle_1.is_ok());

    let candle_3 = upbit::api_quotation::get_candle_minute("KRW-ETH", None, 1, CandleMinute::Min3).await;

    assert!(candle_3.is_ok());

    let candle_5 = upbit::api_quotation::get_candle_minute("KRW-ETH", None, 1, CandleMinute::Min5).await;

    assert!(candle_5.is_ok());

    let candle_10 = upbit::api_quotation::get_candle_minute("KRW-ETH", None, 1, CandleMinute::Min10).await;

    assert!(candle_10.is_ok());

    let candle_15 = upbit::api_quotation::get_candle_minute("KRW-ETH", None, 1, CandleMinute::Min15).await;

    assert!(candle_15.is_ok());

    let candle_30 = upbit::api_quotation::get_candle_minute("KRW-ETH", None, 1, CandleMinute::Min30).await;

    assert!(candle_30.is_ok());

    let candle_60 = upbit::api_quotation::get_candle_minute("KRW-ETH", None, 1, CandleMinute::Min60).await;
